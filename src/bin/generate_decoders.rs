...
fn rename_decoder_folder(original_name: &str) -> Result<(), String> {
    ...
    let original_path = PathBuf::from(format!("{}/{}", DECODER_OUTPUT_DIR, expected_generated_name));
    ...
    if original_path.exists() && expected_generated_name != valid_name {
        println!("🛠 Renaming `{}` to `{}`...", expected_generated_name, valid_name);

        // Try renaming
        match fs::rename(&original_path, &new_path) {
            Ok(_) => println!("✅ Successfully renamed `{}` to `{}`", expected_generated_name, valid_name),
            Err(e) => return Err(format!("❌ Failed to rename `{}` to `{}`: {}", expected_generated_name, valid_name, e)),
        }
    } else {
        println!("⚠️ Folder `{}` does not exist or name is already valid.", expected_generated_name);
    }
...
fn update_mod_rs() -> Result<(), String> {
    ...
    let mut mod_file = fs::File::create(&mod_file_path).map_err(|e| format!("Failed to create mod.rs: {}", e))?;

    let mut mod_content = String::new();
    let entries = fs::read_dir(DECODER_OUTPUT_DIR).map_err(|e| format!("Failed to read decoder directory: {}", e))?;
    ...
    mod_file.write_all(mod_content.as_bytes()).map_err(|e| format!("Failed to write to mod.rs: {}", e))?;
...
fn main() {
    ...
    if Path::new(&format!("{}/{}", DECODER_OUTPUT_DIR, valid_name)).exists() ||
       Path::new(&format!("{}/{}", DECODER_OUTPUT_DIR, expected_generated_name)).exists() {
