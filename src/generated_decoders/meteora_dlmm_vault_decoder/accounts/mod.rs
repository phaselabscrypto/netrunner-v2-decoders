...
impl<'a> AccountDecoder<'a> for AlphaVaultDecoder {
    ...
    fn decode_account(
        ...
        if let Some(decoded_account) =
            merkle_root_config::MerkleRootConfig::deserialize(account.data.as_slice())
        {
            return Some(carbon_core::account::DecodedAccount {
                lamports: account.lamports,
        ...
        if let Some(decoded_account) =
            prorata_vault_config::ProrataVaultConfig::deserialize(account.data.as_slice())
        {
            return Some(carbon_core::account::DecodedAccount {
                lamports: account.lamports,
        ...
        if let Some(decoded_account) =
            fcfs_vault_config::FcfsVaultConfig::deserialize(account.data.as_slice())
        {
            return Some(carbon_core::account::DecodedAccount {
                lamports: account.lamports,
