...
impl<'a> AccountDecoder<'a> for UnstakeDecoder {
    ...
    fn decode_account(
        ...
        if let Some(decoded_account) =
            flash_loan_fee::FlashLoanFee::deserialize(account.data.as_slice())
        {
            return Some(carbon_core::account::DecodedAccount {
                lamports: account.lamports,
        ...
        if let Some(decoded_account) =
            protocol_fee::ProtocolFee::deserialize(account.data.as_slice())
        {
            return Some(carbon_core::account::DecodedAccount {
                lamports: account.lamports,
        ...
        if let Some(decoded_account) =
            stake_account_record::StakeAccountRecord::deserialize(account.data.as_slice())
        {
            return Some(carbon_core::account::DecodedAccount {
                lamports: account.lamports,
