...
#[carbon(discriminator = "0x87cfe470f70f1d96")]
pub struct SwapAndAddCollateral {
    pub params: SwapAndAddCollateralParams,
}

pub struct SwapAndAddCollateralInstructionAccounts {
...
impl carbon_core::deserialize::ArrangeAccounts for SwapAndAddCollateral {
    type ArrangedAccounts = SwapAndAddCollateralInstructionAccounts;

    fn arrange_accounts(
        accounts: &[solana_sdk::instruction::AccountMeta],
    ) -> Option<Self::ArrangedAccounts> {
        let [owner, fee_payer, funding_account, transfer_authority, perpetuals, pool, receiving_custody, receiving_custody_oracle_account, receiving_custody_token_account, position, market, target_custody, target_oracle_account, collateral_custody, collateral_oracle_account, collateral_custody_token_account, funding_token_program, event_authority, program, ix_sysvar, funding_mint, _remaining @ ..] =
            accounts
        else {
            return None;
        };

        Some(SwapAndAddCollateralInstructionAccounts {
            owner: owner.pubkey,
