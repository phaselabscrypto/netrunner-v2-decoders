...
impl<'a> AccountDecoder<'a> for YvaultsDecoder { 
    type AccountType = YvaultsAccount;
     fn decode_account( &self, account: &solana_sdk::account::Account, ) -> Option<carbon_core::account::DecodedAccount<Self::AccountType>> { 
            ...
            if let Some(decoded_account) = whirlpool_strategy::WhirlpoolStrategy::deserialize(account.data.as_slice()) { 
            return Some(carbon_core::account::DecodedAccount { 
                lamports: account.lamports, 
            ...
            if let Some(decoded_account) = collateral_infos::CollateralInfos::deserialize(account.data.as_slice()) { 
            return Some(carbon_core::account::DecodedAccount { 
                lamports: account.lamports, 
            ...
            if let Some(decoded_account) = scope_chain_account::ScopeChainAccount::deserialize(account.data.as_slice()) { 
            return Some(carbon_core::account::DecodedAccount { 
                lamports: account.lamports, 
            ...
            if let Some(decoded_account) = terms_signature::TermsSignature::deserialize(account.data.as_slice()) { 
            return Some(carbon_core::account::DecodedAccount { 
                lamports: account.lamports, 
