...
impl<'a> AccountDecoder<'a> for JitoTipDistributionDecoder { 
    type AccountType = JitoTipDistributionAccount;
     fn decode_account( &self, account: &solana_sdk::account::Account, ) -> Option<carbon_core::account::DecodedAccount<Self::AccountType>> { 
            ...
            if let Some(decoded_account) = tip_distribution_account::TipDistributionAccount::deserialize(account.data.as_slice()) { 
            return Some(carbon_core::account::DecodedAccount { 
                lamports: account.lamports, 
            ...
            if let Some(decoded_account) = claim_status::ClaimStatus::deserialize(account.data.as_slice()) { 
            return Some(carbon_core::account::DecodedAccount { 
                lamports: account.lamports, 
