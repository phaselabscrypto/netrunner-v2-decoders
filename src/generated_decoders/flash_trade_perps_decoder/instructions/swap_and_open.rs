...
#[carbon(discriminator = "0x1ad12a00a93e1e76")]
pub struct SwapAndOpen {
    pub params: SwapAndOpenParams,
}

pub struct SwapAndOpenInstructionAccounts {
...
impl carbon_core::deserialize::ArrangeAccounts for SwapAndOpen {
    type ArrangedAccounts = SwapAndOpenInstructionAccounts;

    fn arrange_accounts(
        accounts: &[solana_sdk::instruction::AccountMeta],
    ) -> Option<Self::ArrangedAccounts> {
        let [owner, fee_payer, funding_account, transfer_authority, perpetuals, pool, receiving_custody, receiving_custody_oracle_account, receiving_custody_token_account, position, market, target_custody, target_oracle_account, collateral_custody, collateral_oracle_account, collateral_custody_token_account, system_program, funding_token_program, event_authority, program, ix_sysvar, funding_mint, collateral_mint, collateral_token_program, _remaining @ ..] =
            accounts
        else {
            return None;
        };

        Some(SwapAndOpenInstructionAccounts {
            owner: owner.pubkey,
