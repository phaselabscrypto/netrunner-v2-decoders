...
pub struct OcpWithdrawSellInstructionAccounts {
...
impl carbon_core::deserialize::ArrangeAccounts for OcpWithdrawSell {
    ...
    fn arrange_accounts(accounts: &[solana_sdk::instruction::AccountMeta]) -> Option<Self::ArrangedAccounts> {
        ...
        let asset_metadata = accounts.get(4)?;
        let asset_token_account = accounts.get(5)?;
        let sellside_escrow_token_account = accounts.get(6)?;
        let buyside_sol_escrow_account = accounts.get(7)?;
        let allowlist_aux_account = accounts.get(8)?;
        let sell_state = accounts.get(9)?;
        let ocp_mint_state = accounts.get(10)?;
        let ocp_policy = accounts.get(11)?;
        let ocp_freeze_authority = accounts.get(12)?;
        let ocp_program = accounts.get(13)?;
        let cmt_program = accounts.get(14)?;
        ...
        Some(OcpWithdrawSellInstructionAccounts {
            owner: owner.pubkey,
