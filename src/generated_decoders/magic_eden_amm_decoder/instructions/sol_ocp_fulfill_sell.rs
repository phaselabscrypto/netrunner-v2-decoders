...
#[carbon(discriminator = "0xd5283a63816df593")]
pub struct SolOcpFulfillSell{
    pub args: SolOcpFulfillSellArgs,
}

pub struct SolOcpFulfillSellInstructionAccounts {
...
impl carbon_core::deserialize::ArrangeAccounts for SolOcpFulfillSell {
    ...
    fn arrange_accounts(accounts: &[solana_sdk::instruction::AccountMeta]) -> Option<Self::ArrangedAccounts> {
        ...
        let ocp_mint_state = accounts.get(12)?;
        let ocp_policy = accounts.get(13)?;
        let ocp_freeze_authority = accounts.get(14)?;
        let ocp_program = accounts.get(15)?;
        let cmt_program = accounts.get(16)?;
        let instructions = accounts.get(17)?;
        let system_program = accounts.get(18)?;
        let token_program = accounts.get(19)?;
        let associated_token_program = accounts.get(20)?;
        let rent = accounts.get(21)?;

        Some(SolOcpFulfillSellInstructionAccounts {
            payer: payer.pubkey,
            owner: owner.pubkey,
