...
#[carbon(discriminator = "0x07a68aabceabecf4")]
...
impl carbon_core::deserialize::ArrangeAccounts for InitializePermissionlessConstantProductPoolWithConfig {
    type ArrangedAccounts = InitializePermissionlessConstantProductPoolWithConfigInstructionAccounts;
    ...
    fn arrange_accounts(accounts: Vec<solana_sdk::instruction::AccountMeta>) -> Option<Self::ArrangedAccounts> {
        ...
        let payer_token_a = accounts.get(13)?;
        let payer_token_b = accounts.get(14)?;
        let payer_pool_lp = accounts.get(15)?;
        let protocol_token_a_fee = accounts.get(16)?;
        let protocol_token_b_fee = accounts.get(17)?;
        let payer = accounts.get(18)?;
        let rent = accounts.get(19)?;
        let mint_metadata = accounts.get(20)?;
        let metadata_program = accounts.get(21)?;
        let vault_program = accounts.get(22)?;
        let token_program = accounts.get(23)?;
        let associated_token_program = accounts.get(24)?;
        let system_program = accounts.get(25)?;

        Some(InitializePermissionlessConstantProductPoolWithConfigInstructionAccounts {
            pool: pool.pubkey,
