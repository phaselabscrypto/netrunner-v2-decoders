...
impl<'a> AccountDecoder<'a> for ValidatorBondsDecoder { 
    type AccountType = ValidatorBondsAccount;
     fn decode_account( &self, account: &solana_sdk::account::Account, ) -> Option<carbon_core::account::DecodedAccount<Self::AccountType>> { 
            ...
            if let Some(decoded_account) = settlement_claim::SettlementClaim::deserialize(account.data.as_slice()) { 
            ...
            if let Some(decoded_account) = settlement_claims::SettlementClaims::deserialize(account.data.as_slice()) { 
            return Some(carbon_core::account::DecodedAccount { 
                lamports: account.lamports, 
            ...
            if let Some(decoded_account) = withdraw_request::WithdrawRequest::deserialize(account.data.as_slice()) { 
            return Some(carbon_core::account::DecodedAccount { 
                lamports: account.lamports, 
